...
실제 동물 등록·수정은 [B-05 관리 API](dog-management-api.md)에서 제공해. 이번 `/access` API는 관리 화면에 들어갈 수 있는지 확인하는 용도이고, 이후 등록·수정 요청에서도 서버가 권한을 다시 검사해야 해.
...
이번에 열어둔 경로·메서드 외에는 기본 차단이야. 상태 확인은 계속 공개하고, B-05의 강아지 관리 경로는 토큰과 보호소 권한이 필요해. 보호소 자체의 등록·승인·소속 변경 API는 아직 없어. RN 웹의 CORS 허용 주소는 실제 프론트 주소가 정해진 뒤 추가해.
