...
Render가 지정한 `PORT`를 받고 `0.0.0.0`에서 요청을 받아. 컨테이너는 Java 힙에 가용 메모리의 최대 50%를 사용해. 나머지는 클래스 정보, 스레드, 네트워크 등에 필요해. Docker 빌드에는 허용한 서버 소스만 전달하고, 최종 이미지에는 JRE, 실행 JAR와 B-16의 Python 모션 렌더러 환경을 넣어. `.env` 파일, 샘플 데이터 로더, 테스트 도구는 배포 이미지에 들어가지 않아.
...
[모션 하네스 연결](motion-harness-pipeline.md)부터 런타임에 Python·Pillow·numpy가 포함돼. V4/V5와 비공개 `dog-assets` 버킷이 필요해. 생성 스위치·PixelLab 키·Supabase 서버 키는 [설정 예시](../backend/.env.assets.example)를 따라 별도로 설정해. 기존 V3 배포 기록은 위의 B-14 시점 결과이며, B-16 실제 적용 결과는 PR·작업 카드에 남겨.
