...
PixelLab을 사용한다. 사진을 참고해 기준 도트를 만들고, 같은 도트에서 8개 동작을 생성한다. 서버에서 한 번 만들어 저장한 PNG를 앱이 재생하는 구조다. 움직일 때마다 AI를 호출하지 않는다.

현재 코드에는 생성 작업 저장, PixelLab 호출, 비공개 Storage 저장, 보호소 검토, 공개 조회까지 구현돼 있다. 실제 크롤러는 실행하지 않았다. Supabase 개발 DB의 V4 적용과 Render 배포·기능 활성화는 별도이며, 모든 생성 스위치는 기본 OFF다.

## 생성 규격

기준 도트는 `generate-image-v2`, 동작은 Animation tools의 **Animate with text (Pro)**인 `animate-with-text-v2`를 사용한다. 같은 기준 도트, 64×64, 투명 배경, 오른쪽 3/4 방향을 유지한다. Pro는 이 크기에서 동작별 16프레임을 반환한다. 코드 버전은 `pixellab-pro64-v2`다.

| 키 | 동작 | 프레임당 시간 | 재생 |
| --- | --- | --- | --- |
| IDLE | 서서 숨 쉬고 살펴보기 | 140ms | 반복 |
| WALK | 네 발로 걷기 | 75ms | 반복 |
| RUN | 달리기 | 55ms | 반복 |
| SNIFF | 고개를 숙여 냄새 맡기 | 100ms | 반복 |
| TAIL_WAG | 꼬리 흔들기 | 90ms | 반복 |
| BACK_OFF | 바라보는 방향을 유지하며 물러서기 | 90ms | 반복 |
...
전체 프레임이 16장인지, 투명도·64px 캔버스·잘린 외곽이 없는지 검사한다. 기계 검사는 해부학이나 자연스러운 걸음을 보장하지 않으므로 생성 결과는 `REVIEW`까지만 자동으로 진행된다. 보호소가 외형·동작을 확인해야 공개할 수 있다. 성격·행동 비중은 사진에서 추정하지 않고 기존 [행동 설정](dog-behavior-api.md)을 사용한다.
...
5. 워커가 기준 도트 → 8종을 순서대로 만들고 비공개 `dog-assets`에 저장한다. 같은 사진 ID·생성 버전의 중복 이벤트는 기존 작업을 돌려준다.
...
| GET `/shelter-admin/dogs/{dogId}/assets/{id}` | 해당 보호소 소속 | 9단계의 진행 상태·실패 코드. 200 |
...
상태는 `QUEUED → RUNNING → REVIEW → APPROVED/REJECTED`다. 오류 상태는 `FAILED`, `OUTCOME_UNKNOWN`, `CANCELLED`로 나뉜다. 조회에는 BASE와 8종의 상태가 함께 들어간다. 사진 원본 주소나 생성 업체 키는 응답에 포함하지 않는다.
...
예시의 배열은 한 프레임만 표시했다. 실제로는 8개 동작과 동작별 16개 좌표가 온다. 한 시트는 1024×64다. 앱은 64×64 영역을 잘라 재생하고 확대 시 nearest-neighbor를 사용한다. 서명 주소는 60초이므로 만료되면 manifest를 다시 조회한다. 이미 내려받은 파일 자체를 회수하는 기능은 아니다.
...
DB는 [V4](../backend/src/main/resources/db/migration/V4__dog_asset_generation.sql)를 기존 V1~V3 다음에 Flyway로 적용한다. 새 테이블 5개 모두 RLS와 클라이언트 접근 차단을 적용한다. 기존 사진을 허가하거나 자동 생성 대상으로 바꾸는 데이터 변경은 없다. `exportSupabaseUpgrade`는 기존 V1→V3 전용 도구이므로 V4를 적용하지 않는다.
...
남은 서비스 연결은 수집 허가 확보, 수집기의 사진 저장·등록 훅 호출, 개발 DB V4와 비공개 에셋 버킷 적용, 환경변수·워커 활성화, RN 화면에서의 재생 검수다. 이번 에셋은 한 방향이며 4/8방향 생성은 포함하지 않는다.
