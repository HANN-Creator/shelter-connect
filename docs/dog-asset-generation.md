...
기준 도트는 `generate-image-v2`, 동작은 Animation tools의 **Animate with text (Pro)**인 `animate-with-text-v2`를 사용한다. 같은 기준 도트, 64×64, 투명 배경, 오른쪽 3/4 방향을 유지한다. Pro는 이 크기에서 동작별 16프레임을 반환한다. 현재 생성 버전은 `pixellab-harness-v3`다. 아래 표는 선택 가능한 행동이며 전부 생성하는 목록은 아니다. WALK/RUN/BACK_OFF는 로컬 하네스가 24프레임으로 만든다.
