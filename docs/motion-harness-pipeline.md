...
## 걷기 왜곡과 외곽선 덧칠 수정 (0.3.1)

0.3.0은 다리 시작점과 발 위치 사이에 고정 길이 관절을 넣어, 짧거나 이미 굽은 원본 다리가 과하게 접히는 문제가 있었다. 또한 모든 실루엣을 한 `LINE` 색으로 다시 칠해 원래의 밝고 얇은 선을 덮었다. 프레임 수·팔레트 검사 통과가 자연스러운 움직임을 뜻하지 않는 사례다.

WALK는 원본에 그려진 다리 모양을 기준으로 작은 변위를 적용한다. 발이 바닥에 닿은 구간과 들어 올리는 구간을 나누고, 짧은 다리는 보폭과 들어 올리는 높이를 줄인다. 픽셀 행이 서로 접히지 않는 역변환으로 다리를 움직여 발목 경계에서 폭이 갑자기 늘어나지 않게 한다. 발끝은 같은 모양으로 이동한다. 몸의 상하 이동과 회전도 작은 보행 범위로 줄였다. 걷기는 48×30ms 그대로다.

세 로컬 동작의 외곽선 처리는 원본 실루엣 픽셀의 위치를 함께 추적한다. 기존 외곽선의 색과 굵기는 보존하고, 부위를 잘라 움직여 새로 노출된 경계만 잇는다. 원본 BASE 파일·사진 기반 생성 요청은 변경하지 않는다. 서로 다른 시점에 새로 생성된 BASE는 같은 사진이어도 그림이 달라질 수 있으므로 비교 화면에는 원본 파일을 명시한다.

검증은 원본 자세의 픽셀 일치, 발끝 무늬 보존, 다리 행 폭·접지·들림 범위, 짧은 다리, 외곽선 원본 색과 잘린 경계 복원으로 구분한다. API·DB·프레임 규격 변경은 없다. 표시할 때는 한 프레임을 정수 배율로 확대하고 큰 화면 전체에 늘려 붙이지 않는다. 64px 그림을 크게 보여준다고 디테일이 늘어나지는 않는다.
