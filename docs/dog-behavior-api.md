...
프론트에서 행동별로 구현할 이동·진입·종료·대체 규칙은 [강아지 행동별 재생 명세서](dog-action-playback-spec.md)를 봐줘. 아래는 API 계약이고, 재생 명세서의 상황 우선순위와 전환은 RN 구현 초안이야.
...
`chaseEnabled=false`면 공을 쫓지 않고, 이때 `returnEnabled=true`는 허용하지 않아. 공을 쫓을 때 RUN 비중이 0이거나 RUN 에셋이 없으면 사용 가능한 WALK를 검토해. 에셋 로딩·비중·cooldown 조건을 만족하는 이동이 없으면 추적을 시작하지 않아. returnEnabled가 켜져 있으면 공 획득 뒤 입 위치에 공을 붙여 돌아오고, 꺼져 있으면 가져오기 단계는 생략해. 이동·공 충돌과 복귀 경로는 앱에서 처리해.
...
설정이 없거나 초안이거나, 확인 후 근거가 철회되거나, 저장된 형식이 지원되지 않으면 `basis:DEFAULT`, `revision:null`로 반환해. 기본값은 IDLE 60 / WALK 30 / SIT 10 / 나머지 0, 접근·거리 0, 공놀이 꺼짐이야. 이동 속도는 WALK 0.8 / RUN 1.8 / BACK_OFF 0.6이며 RUN과 BACK_OFF는 비중 0이라 재생하지 않아. 유지 2~5초·재선택 대기 2초·반응 대기 1초를 사용해. 기본 설정은 DB에 자동 저장하지 않아.
...
2026.09.21 B-10A에서 지정 Supabase 개발 프로젝트에 V2·V3를 적용하고 검증했어. [실제 DB 확인 결과](supabase-development-db.md)를 보면 돼. 다른 DB는 현재 버전과 대상을 확인한 뒤 적용해야 해. 새 환경변수나 외부 AI 키는 필요 없어.

API와 지정 개발 DB까지 준비됐어. RN에서 8종 애니메이션을 재생하는 작업은 프론트의 캐릭터·재생 구조가 준비된 뒤 연결해. AI 초안 자동 생성과 실제 서버 배포도 별도야. 샘플 강아지에 확인된 행동을 임의로 등록하지 않았으므로 샘플 조회는 DEFAULT로 시작해.
