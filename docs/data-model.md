...
사용자 메시지 하나에 완료된 답변은 최대 1개다. 같은 요청을 다시 보내면 기존 요청을 조회하고, 내용이 다르면 충돌로 처리한다. DB의 UNIQUE만으로 외부 AI 호출·과금까지 한 번이 되는 것은 아니다. B-06은 사용자 메시지 저장·동일 요청 재전송을 처리한다. AI 작업 선점, 실패 복구, 답변과 요청 상태의 원자적 저장은 B-07에서 구현한다.

B-06의 새 메시지는 PENDING으로 저장하고 기존 메시지 재전송으로 상태를 변경하지 않는다. B-07에서는 실패한 요청을 FAILED로 남기고 명시적인 답변 재시도 때만 같은 행을 PENDING으로 갱신할 예정이다. 아직 생성되지 않은 AI 답변을 빈 본문으로 저장하지 않는다.
