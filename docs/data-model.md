...
대화방은 `user_id`, `dog_id`, `status`(OPEN / CLOSED)를 가진다. 한 사용자가 같은 강아지와 여러 대화방을 만들 수 있다. B-06 API는 같은 사용자·강아지의 가장 최근 생성된 OPEN 방을 재사용하고, 없을 때 생성한다. CLOSED 방의 기록은 보존한다.
...
| `role` | USER / ASSISTANT. B-06 API도 대문자 사용 |
| `content` | 비어 있지 않은 본문. DB 상한 12,000자. B-06 사용자 입력은 4,000자, 화면 안내는 C-03에서 검토 |
...
사용자 메시지 하나에 완료된 답변은 최대 1개다. 같은 요청을 다시 보내면 기존 요청을 조회하고, 내용이 다르면 충돌로 처리한다. DB의 UNIQUE만으로 외부 AI 호출·과금까지 한 번이 되는 것은 아니다. B-06은 사용자 메시지 저장·동일 요청 재전송을 처리한다. B-07은 V2의 선점 토큰·만료 시각·시도 횟수로 동시 생성과 실패 복구를 처리하고, 답변·근거·요청 완료 상태를 한 트랜잭션으로 저장한다.

B-06의 새 메시지는 PENDING으로 저장하고 기존 메시지 재전송으로 상태를 변경하지 않는다. B-07은 실패한 요청을 FAILED로 남기고 명시적인 답변 재시도 때만 같은 행을 PENDING으로 갱신한다. 최초 포함 최대 3회 시도하며, 만료된 작업은 다음 답변 요청에서 복구한다. 아직 생성되지 않은 AI 답변을 빈 본문으로 저장하지 않는다.
...
B-07은 생성 입력에 같은 강아지의 CONFIRMED 관찰만 넣고, 저장 직전 인용 기록의 상태·내용·수정 시각을 다시 확인한다. `observation_snapshot`은 당시 id·category·content·observedAt·updatedAt을 JSON 문자열로 보관한다. DB는 같은 강아지인지 보장하고 서버는 인용 ID를 검증하지만 문장의 의미까지 정확함을 보장하지는 않는다. [답변 생성 규칙](grounded-chat-api.md)을 참고한다.

### `adoption_notes`

`user_id`, `dog_id`당 하나의 메모. `questions`, `care_plan`은 텍스트, `checklist`는 JSON 객체로 저장한다. 빈 메모를 허용한다. B-09 API는 질문 5,000자·돌봄 계획 10,000자와 5개 준비 체크 키의 boolean 값을 검증한다.

실제 입양 신청서나 제출 완료 기록은 아니다. [B-09 개인 메모 API](adoption-notes-api.md)는 본인만 저장·조회하며 보호소·운영자도 소유권을 우회하지 않는다. 첫 저장에는 공개·입양 가능 강아지만 허용하고 기존 개인 메모는 공개 상태 변경 뒤에도 읽고 수정할 수 있다. `expectedUpdatedAt`과 사용자·메모 잠금으로 동시 생성·오래된 수정의 덮어쓰기를 차단한다. 기존 V1 구조를 그대로 사용한다.
