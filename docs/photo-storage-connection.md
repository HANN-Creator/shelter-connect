...
2026.09.21에 지정 개발 프로젝트의 **비공개 사진 저장소 연결과 실제 이미지 다운로드까지 확인했어.** 사용자가 로컬 설정 파일에 저장한 서버 키를 사용했고, 키와 서명 URL은 출력하거나 커밋하지 않았어.

| 확인 항목 | 결과 |
| --- | --- |
| 사진 저장소 | 비공개 `dog-photos` 생성, 최대 5 MiB·JPEG/PNG/WebP/GIF 제한 확인 |
| 실제 파일 | 전용 테스트 PNG 1개 업로드, 앱 사진 행은 기존처럼 0개 |
| 서버 저장소 코드 | 실제 `SupabasePhotoStorage`로 60초 서명 URL 발급 성공 |
| 다운로드 | 서명 URL에서 키 없이 받은 PNG가 원본과 바이트 단위로 일치, image/png 확인 |
| 우회 접근 | 공개 URL·미인증 직접 다운로드 차단 확인 |
...
| 사진 기능을 켠 서버 | Supabase DB 연결 성공, readiness 200 / UP, 보호소·강아지 조회 및 미인증 30개 경로 차단 통과 |

버킷과 테스트 파일만 새로 만들었어. 저장소 정책과 기존 앱 데이터는 변경하지 않았고, DB의 상태 확인은 읽기 전용으로 했고 확인용 서버는 종료했어. 실제 로그인 사용자의 대화 → 사진 API 전체 흐름, 보호소 사진 등록과 배포는 별도야.
