...
검증할 때는 JDBC 연결을 읽기 전용으로 열고 기존 공개 조회·미인증 검사만 실행했어. 마이그레이션·SQL 초기화·AI·Storage 호출은 끄고 확인용 서버는 검사 후 종료했어. DB 비밀번호·데이터·RLS·계정 권한을 변경하지 않았어. 이후 B-12에서 [사진 저장소 연결](photo-storage-connection.md), B-13에서 [실제 로그인·개인 기록 저장과 재시작 후 보존](login-storage-verification.md)을 확인했어. AI 호출, 외부 배포와 RN 화면 연결은 남아 있어.
