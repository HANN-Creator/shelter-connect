...
사진과 특징을 넣었을 때 실제 서버가 동작 에셋까지 만드는지는 [실서버 생성 검증](../docs/live-asset-verification.md)으로 확인할 수 있어. 테스트용 입력을 API로 보내고, 서버가 만든 PNG와 프론트 조회 응답을 검사해. 일반 CI에서는 유료 호출 없이 검증 도구만 검사해.
