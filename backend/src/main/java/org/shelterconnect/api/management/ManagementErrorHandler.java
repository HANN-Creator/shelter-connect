...
public class ManagementErrorHandler {
	private static final Logger log = LoggerFactory.getLogger(ManagementErrorHandler.class);
	@ExceptionHandler(ManagementException.class)
	ResponseEntity<SecurityErrors.Error> invalid(ManagementException exception, HttpServletRequest request) {
	...
	ResponseEntity<SecurityErrors.Error> unexpected(Exception exception, HttpServletRequest request) {
		log.error("Management request failed; requestId={}", ApiRequestFilter.requestId(request), exception);
